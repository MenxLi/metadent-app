<template>
  ...
  >
    ...
    />
    <div v-if="userStore.settings.showImageLabelerHint" class="pointer-events-none">
      <div
        v-if="!isHintCollapsed"
        class="pointer-events-none absolute top-2 left-2 right-2 z-10 sm:right-auto"
      >
        <div class="max-w-60 overflow-hidden rounded-lg border border-white/10 bg-stone-950/30 text-white shadow-md backdrop-blur-sm">
      ...
      <div v-else class="pointer-events-none absolute top-2 left-2 z-10">
        <FloatingIconButton
    ...
    >
      <ImageLabelerProposalHints
        :region-proposal-bbox-hints="props.regionProposalBboxHints"
...
watch(
  ...
  (labels) => {
    const updated = labels.map(label => {
      if (label.description.trim() === '' && label.autoGenerated != null) {
        return {
          ...label,
          autoGenerated: null,
        };
    ...
    const changed = updated.some((label, index) => label !== labels[index]);
    if (changed) {
...
function removeContour(labelId: string, contourIndex: number) {
  const updatedLabels: LabelItem[] = props.labels.map(label => {
    ...
    return {
      ...label,
      contours: label.contours.filter((_, index) => index !== contourIndex),
