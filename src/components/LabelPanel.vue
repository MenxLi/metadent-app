<script lang="ts" setup>
  ...
  function hasQuestionEndedSentence(text: string): boolean {
    const normalized = text.replace(/\r\n?/g, '\n').trim();
    if (!normalized) {
      return false;
    }

    // Treat any sentence that ends with a question mark as low confidence.
    return /(^|[.!?。！？\n]\s*)[^.!?。！？\n]*[?？](?=\s|$)/.test(normalized);
  }
    ...
    try {
      ...
      descriptions.forEach((description) => {
        ...
        const newItem = makeNewLabelItem(
          description,
          true,
          hasQuestionEndedSentence(description)
        );
        if (nInserted === 0) {
  ...
  async function handleContourCommitted(payload: { label: LabelItem; labels: LabelItem[] }) {
    ...
    try {
      ...
      targetLabel.lowConfidence = hasQuestionEndedSentence(description);
