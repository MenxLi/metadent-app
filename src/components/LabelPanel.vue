<script lang="ts" setup>
  ...
  import { useRegionProposalHints } from '@/composables/useRegionProposalHints';
  ...
  } = useRegionProposalHints({
    activeFileName: computed(() => dataStore.activeDataItem?.fileName),
    labelItems: computed(() => dataStore.activeDataLabel?.items ?? []),
  });
  ...
  function makeNewLabelItem(
    description = '',
    autoGenerated: boolean | null = null,
    lowConfidence = false,
  ): LabelItem {
    ...
    try {
      ...
      const descriptions = (await aiService.regionDescriptionPropose(imageId, activeDataLabel))
        .map((description) => ({
          ...
          lowConfidence: description.low_confidence,
          bboxes: description.bbox ?? [],
        }))
        .filter((description) => Boolean(description.content));
      ...
      descriptions.forEach((description) => {
        if (existingDescriptions.has(description.content)) {
          return;
        }
        const newItem = makeNewLabelItem(
          description.content,
          true,
        ...
        setRegionProposalBboxHintsForLabel(newItem.id, description.bboxes);
  ...
  async function handleContourCommitted(payload: { label: LabelItem; labels: LabelItem[] }) {
    clearRegionProposalBboxHintsForLabel(payload.label.id);
    ...
    try {
      ...
      targetLabel.description = description.content;
      targetLabel.lowConfidence = description.low_confidence;
  ...
  function restoreContourBackup() {
    ...
    clearRegionProposalBboxHintsForLabel(targetLabel.id);
  }
    ...
    <div class="flex justify-center items-center p-4 w-full rounded-lg bg-gray-100" ref="imageContainer">
      <div class="relative" v-if="dataStore.activeDataItem && dataStore.activeDataLabel">
        ...
        <ImageLabeler
          ...
          :max-height="600"
          :max-width="imageMaxW"
          :region-proposal-bbox-hints="regionProposalBboxHints"
    ...
    <div v-if="dataStore.activeDataItem && dataStore.activeDataLabel" class="flex flex-col gap-3">
      ...
      >
        <LabelItemInput
          ...
          @delete="(id: string) => {
            clearRegionProposalBboxHintsForLabel(id);
