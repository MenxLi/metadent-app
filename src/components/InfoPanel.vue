...
<template>
  ...
  <div v-if="dataInfo" class="flex items-center justify-between bg-white shadow-md p-4 rounded-lg">
    ...
    class="flex items-center justify-center gap-2 bg-red-50 p-2 rounded-xl">
      <div class="flex items-center justify-center w-10 h-10 bg-red-200 rounded-full" v-if="dataStore.activeDataLock[1].lockedBy">
        <svg class="w-6 h-6 text-red-700" fill="none" stroke="currentColor" stroke-width="3.5"
            viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          <path stroke-linecap="round" stroke-linejoin="round"
                d="M16 12V8a4 4 0 10-8 0v4M5 12h14v10H5V12z"/>
        </svg>
      </div>
      <div class="flex items-center justify-center w-10 h-10 bg-red-100 rounded-full" v-else>
        <svg class="w-6 h-6 text-red-800" fill="none" stroke="currentColor" stroke-width="4.5"
            viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
        </svg>
      </div>

      <span class="text-red-900 text-md font-semibold" v-if="dataStore.activeDataLock[1].lockedBy">
        Locked by {{ dataStore.activeDataLock[1].lockedBy }}
      </span>
      <span class="text-red-900 text-md font-semibold" v-else>
        Lock failed
      </span>
    </div>
