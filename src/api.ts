...
export async function autoGenerateDescription(payload: any) {
  const userStore = useUserStore()
  const settings = userStore.settings

  if (!settings.enableAIAutoGen) {
    console.log('AI auto generation disabled. Switched to manual input.')
    throw new Error('AI auto generation disabled. Switched to manual input.')
  }

  if (!settings.aiBackendUrl) {
    userStore.disableAIAutoGen()
    console.log('AI backend URL not configured. Switched to manual input.')
  ...
  try {
    ...
    userStore.disableAIAutoGen()
    console.log('AI auto generation failed. Switched to manual input.')
    throw new Error('AI auto generation failed. Switched to manual input.')
  }
}
